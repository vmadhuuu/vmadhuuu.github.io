...
export function PublicationItemCard({
  publication,
  compact = false,
}: PublicationItemCardProps) {
  return (
    <article
      className={cn(
    ...
    >
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
        {publication.image && (
          <div
            className={cn(
              ...
              compact
                ? "h-36 w-full sm:h-36 sm:w-60"
                : "h-44 w-full sm:h-44 sm:w-72",
            )}
          >
            <img
        ...
        <div className="min-w-0 flex-1">
          ...
          <h3
            className={cn(
              "mt-2 leading-tight text-black",
              compact ? "text-[1.18rem]" : "text-[1.42rem]",
            )}
          >
            {publication.title}
          </h3>
          ...
          <p className="mt-1 text-[15px] text-[rgba(17,17,17,0.62)]">
            {publication.venue}
          </p>
...
function getPublicationLinkIcon(label: string) {
  ...
  if (normalized.includes("slides") || normalized.includes("talk"))
    return Presentation;
